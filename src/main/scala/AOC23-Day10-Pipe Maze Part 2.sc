...
val sample1Input = "..........\n.S------7.\n.|F----7|.\n.||....||.\n.||....||.\n.|L-7F-J|.\n.|..||..|.\n.L--JL--J.\n.........."
val sample2Input = ".F----7F7F7F7F-7....\n.|F--7||||||||FJ....\n.||.FJ||||||||L7....\nFJL7L7LJLJ||LJ.L-7..\nL--J.L7...LJS7F-7L7.\n....F-J..F7FJ|L7L7L7\n....L7.F7||L7|.L7L7|\n.....|FJLJ|FJ|F7|.LJ\n....FJL-7.||.||||...\n....L---J.LJ.LJLJ..."
val sample3Input = "FF7FSF7F7F7F7F7F---7\nL|LJ||||||||||||F--J\nFL-7LJLJ||||||LJL-77\nF--JF--7||LJLJ7F7FJ-\nL---JF-JLJ.||-FJLJJ7\n|F|F-JF---7F7-L7L|7|\n|FFJF7L7F-JF7|JL---7\n7-L-JL7||F7|L7F-7F7|\nL.L7LFJ|||||FJL7||LJ\nL7JLJL-JLJLJL--JLJ.L"
...
//    println(f"isLegalMove: checking next move for $currentPiece")
    ...
    currentPiece match {
      case 'S' =>
        if(direction == "NORTH" && (nextPiece == '|' || nextPiece == 'F' || nextPiece == '7' || nextPiece == 'S'))
//          println(f"isLegalMove: Current piece $currentPiece and moving NORTH to nextPiece $nextPiece")
          return true
        else if (direction == "EAST" && (nextPiece == '-' || nextPiece == '7' || nextPiece == 'J' || nextPiece == 'S'))
//          println(f"isLegalMove: Current piece $currentPiece and moving EAST to nextPiece $nextPiece")
          return true
        else if (direction == "SOUTH" && (nextPiece == '|' || nextPiece == 'L' || nextPiece == 'J' || nextPiece == 'S'))
//          println(f"isLegalMove: Current piece $currentPiece and moving SOUTH to nextPiece $nextPiece")
          return true
        else if (direction == "WEST" && (nextPiece == '-' || nextPiece == 'F' || nextPiece == 'L' || nextPiece == 'S'))
...
//          println(f"isLegalMove: Current piece $currentPiece and moving SOUTH to nextPiece $nextPiece")
      ...
      case '-' =>
        if (direction == "EAST" && (nextPiece == '-' || nextPiece == '7' || nextPiece == 'J' || nextPiece == 'S'))
//          println(f"isLegalMove: Current piece $currentPiece and moving EAST to nextPiece $nextPiece")
          return true
        else if (direction == "WEST" && (nextPiece == '-' || nextPiece == 'F' || nextPiece == 'L' || nextPiece == 'S'))
//          println(f"isLegalMove: Current piece $currentPiece and moving WEST to nextPiece $nextPiece")
...
//          println(f"isLegalMove: Current piece $currentPiece and moving WEST to nextPiece $nextPiece")
      ...
      case '7' =>
        if (direction == "SOUTH" && (nextPiece == '|' || nextPiece == 'L' || nextPiece == 'J' || nextPiece == 'S'))
//          println(f"isLegalMove: Current piece $currentPiece and moving SOUTH to nextPiece $nextPiece")
          return true
        else if (direction == "WEST" && (nextPiece == '-' || nextPiece == 'F' || nextPiece == 'L' || nextPiece == 'S'))
//          println(f"isLegalMove: Current piece $currentPiece and moving WEST to nextPiece $nextPiece")
...
@tailrec
def move(mapMatrix: Array[Array[Char]], currentX: Int, currentY: Int, previousX: Int, previousY: Int, cleanedMapMatrix: Array[Array[Char]]) : Array[Array[Char]] =
  val currentPiece = mapMatrix(currentY)(currentX)
...
//      println(f"move: current piece $currentPiece $currentX $currentY moving NORTH")
      cleanedMapMatrix(currentY)(currentX) = currentPiece
      return move(mapMatrix, currentX, currentY - 1, currentX, currentY, cleanedMapMatrix)
    else if isLegalMove(mapMatrix, previousX, previousY, currentX, currentY, currentX + 1, currentY, "EAST") then
//      println(f"move: current piece $currentPiece $currentX $currentY moving EAST")
      cleanedMapMatrix(currentY)(currentX) = currentPiece

      return move(mapMatrix, currentX + 1, currentY, currentX, currentY, cleanedMapMatrix)
    else if isLegalMove(mapMatrix, previousX, previousY, currentX, currentY, currentX, currentY + 1, "SOUTH") then
//      println(f"move: current piece $currentPiece $currentX $currentY moving SOUTH")
      cleanedMapMatrix(currentY)(currentX) = currentPiece

      return move(mapMatrix, currentX, currentY + 1, currentX, currentY, cleanedMapMatrix)
    else if isLegalMove(mapMatrix, previousX, previousY, currentX, currentY, currentX - 1, currentY, "WEST") then
//      println(f"move: current piece $currentPiece $currentX $currentY moving WEST")
      cleanedMapMatrix(currentY)(currentX) = currentPiece
      return move(mapMatrix, currentX - 1, currentY, currentX, currentY, cleanedMapMatrix)
    else
...
def floodFill(mapMatrix: Array[Array[Char]], pos_x: Int, pos_y: Int) : Unit =
  if (mapMatrix(pos_y)(pos_x) eq '|')
    || (mapMatrix(pos_y)(pos_x) eq 'S')
    || (mapMatrix(pos_y)(pos_x) eq '-')
    || (mapMatrix(pos_y)(pos_x) eq 'F')
    || (mapMatrix(pos_y)(pos_x) eq '7')
    || (mapMatrix(pos_y)(pos_x) eq 'L')
    || (mapMatrix(pos_y)(pos_x) eq 'J')
    || (mapMatrix(pos_y)(pos_x) eq ' ')
    || (mapMatrix(pos_y)(pos_x) eq 'X')
  then
    return ()// already go back // if there is no wall or if i haven't been there

  mapMatrix(pos_y)(pos_x) = ' ' // mark the point so that I know if I passed through it.

  floodFill(mapMatrix, pos_x + 1, pos_y) // east
  floodFill(mapMatrix, pos_x - 1, pos_y) // west
  floodFill(mapMatrix, pos_x, pos_y + 1) // south
  floodFill(mapMatrix, pos_x, pos_y - 1) // north

  return ()

def setupMatrixForFloodFill(matrixHeight: Int, matrixWidth: Int) : Array[Array[Char]] =
  val cleanedMatrix = Array.ofDim[Char](matrixHeight, matrixWidth)

  for (i <- 0 until matrixHeight) {
  ...
  for (i <- 0 until matrixHeight) {
    cleanedMatrix(i)(0) = 'X'
    cleanedMatrix(i)(matrixWidth - 1) = 'X'
  }

  for (i <- 0 until matrixWidth) {
    cleanedMatrix(0)(i) = 'X'
    cleanedMatrix(matrixHeight - 1)(i) = 'X'
  }
...
@tailrec
def getBoundariesScanningNorth(matrix: Array[Array[Char]], startingX: Int, startingY: Int, boundaries: Int): Int =
  val currentPiece = matrix(startingY)(startingX)
//  println(f"x $startingX y $startingY $currentPiece")
  if(currentPiece == 'X') then
    boundaries
  else
    if currentPiece == '-'
      || currentPiece == 'F'
      ...
      || currentPiece == 'J'
      || currentPiece == 'S' then
      getBoundariesScanningNorth(matrix, startingX, startingY-1, boundaries+1)
    else
      getBoundariesScanningNorth(matrix, startingX, startingY-1, boundaries)

@tailrec
def getBoundariesScanningSouth(matrix: Array[Array[Char]], startingX: Int, startingY: Int, boundaries: Int): Int =
  val currentPiece = matrix(startingY)(startingX)
  //  println(f"x $startingX y $startingY $currentPiece")
  if(currentPiece == 'X') then
    boundaries
    ...
    if currentPiece == '-'
      ...
      || currentPiece == 'J'
      || currentPiece == 'S' then
      getBoundariesScanningSouth(matrix, startingX, startingY+1, boundaries+1)
    else
      getBoundariesScanningSouth(matrix, startingX, startingY+1, boundaries)

@tailrec
def getBoundariesScanningEast(matrix: Array[Array[Char]], startingX: Int, startingY: Int, boundaries: Int): Int =
  val currentPiece = matrix(startingY)(startingX)
  //  println(f"x $startingX y $startingY $currentPiece")
  if(currentPiece == 'X') then
    boundaries
    ...
    if currentPiece == '|'
      || currentPiece == 'L'
      || currentPiece == 'J' then
      getBoundariesScanningEast(matrix, startingX+1, startingY, boundaries+1)
    else
      getBoundariesScanningEast(matrix, startingX+1, startingY, boundaries)

@tailrec
def getBoundariesScanningWest(matrix: Array[Array[Char]], startingX: Int, startingY: Int, boundaries: Int): Int =
  val currentPiece = matrix(startingY)(startingX)
  //  println(f"x $startingX y $startingY $currentPiece")
  if(currentPiece == 'X') then
    boundaries
    ...
    if currentPiece == '|'
      ...
      || currentPiece == '7'
      || currentPiece == 'S' then
      getBoundariesScanningWest(matrix, startingX-1, startingY, boundaries+1)
    else
      getBoundariesScanningWest(matrix, startingX-1, startingY, boundaries)

def floodFillWhereWaterCanSeep(matrix: Array[Array[Char]], matrixHeight: Int, matrixWidth: Int): Unit =
  // find each remaining I
  // then scan vertically or horizontally - lets just scan north, if we hit an even number of boundaries then its outside else inside
  for (y <- 0 until matrixHeight) {
    for (x <- 0 until matrixWidth) {
      if matrix(y)(x) == 'I' then
        val totalBoundariesNorth = getBoundariesScanningNorth(matrix, x, y, 0)
        val totalBoundariesSouth = getBoundariesScanningSouth(matrix, x, y, 0)
        val totalBoundariesEast = getBoundariesScanningEast(matrix, x, y, 0)
        val totalBoundariesWest = getBoundariesScanningWest(matrix, x, y, 0)
//        println(f"total boundaries $totalBoundaries for $x $y ")
//        if totalBoundariesNorth % 2 == 0 && totalBoundariesSouth % 2 == 0 &&totalBoundariesEast % 2 == 0 &&totalBoundariesWest % 2 == 0 then
        if totalBoundariesEast % 2 == 0 then
//          println(f"updating $x $y to O")
          matrix(y)(x) = ' '
//        println()
    }
  }
...
def solution(input: String): Int =
  val inputPaddedTopBottom = "X"*input.indexOf("\n") + "\n"
    + "."*input.indexOf("\n") + "\n"
    + input +  "\n"
    + "."*input.indexOf("\n") + "\n"
    + "X"*input.indexOf("\n")

  val paddedSidesMatrix = inputPaddedTopBottom.linesIterator.map(parse).toArray

  val mapMatrix = Array.ofDim[Char](1,paddedSidesMatrix(0).length) ++ paddedSidesMatrix ++ Array.ofDim[Char](1,paddedSidesMatrix(0).length)

  printMatrix(mapMatrix)

  val (startingX, startingY) = getStartingPosition(mapMatrix)

  val matrixHeight = mapMatrix.length
  val matrixWidth = mapMatrix(0).length

  val cleanedMatrix = setupMatrixForFloodFill(matrixHeight, matrixWidth)

  val updatedCleanMatrix = move(mapMatrix, startingX, startingY, -1, -1, cleanedMatrix)

  floodFill(updatedCleanMatrix, 1, 1)

  printMatrix(updatedCleanMatrix)

  println(updatedCleanMatrix.flatten.count(_ == 'I'))

  floodFillWhereWaterCanSeep(updatedCleanMatrix, matrixHeight, matrixWidth)

  printMatrix(updatedCleanMatrix)

  updatedCleanMatrix.flatten.count(_ == 'I')

println(solution(sample1Input))
println(solution(sample2Input))
println(solution(sample3Input))
println(solution(puzzleInput))

// 271 too low
// 280 too low
// 285 is correct - by guessing
// 289 too high