...
def isValidGameSet( set : List[String] ) : List[Array[(String,Int)]] = {
  val ballColourLimit = Map("red" -> 12, "green" -> 13, "blue" -> 14)

  val setData = List()

  val colorGroups = for (balls <- set) yield {
    val colourGroups = balls.split(",")
//    println("number of colour groups " + colourGroups.length)

    val colorGroupData = for (colourGroup <- colourGroups) yield {
//      println(colourGroup)

      val numberOfBalls = colourGroup.trim.split(" ")(0).toInt
      val ballColor = colourGroup.trim.split(" ")(1)

      (ballColor, numberOfBalls)
    }

//    println("colorGroupData")
//    println(colorGroupData.mkString("Array(", ", ", ")"))
    //    Array((red,1), (green,2), (blue,6))
...
def extractPowers( input : List[String] ) : List[Int] = {
  val powers = for( game <- input ) yield {
    var red = 0;
    var green = 0;
    var blue = 0;
    ...
    val colorGroupData = isValidGameSet(setData)
    //    println("setData " + setData)
    colorGroupData.foreach(i =>
      //      println(("i " + i.mkString("Array(", ", ", ")")))
      i.foreach(colourAndBalls =>
        if (colourAndBalls(0) == "red" && colourAndBalls(1) > red) red = colourAndBalls(1)
        if (colourAndBalls(0) == "green" && colourAndBalls(1) > green) green = colourAndBalls(1)
        if (colourAndBalls(0) == "blue" && colourAndBalls(1) > blue) blue = colourAndBalls(1)
      )
    )
...
val gamePowers = extractPowers(sampleInput)
println("Sum of powers: " + gamePowers.sum)
...
val gamePowers1 = extractPowers(puzzleInput)
println("Sum of powers: " + gamePowers1.sum)
