...
// get every number from the schematic and assume they are all valid
// then scan each number and check if connecting symbol is found
// if not then remove the number from the initial set
// then sum all the part numbers in the remaining set

//case class SchematicLine(partNumbers : List[String])
case class SchematicParts(partNumbers : List[String])

// Summary type alias for any function that has a Game param and returns an Int
//type Summary = SchematicLine => Int

// create a regex with '.r' to find numbers
val numPattern = "[0-9]+".r
val numericPattern = "([0-9]+)".r

def extendSchematic(input: String, lineWidth: Int) : String =
  val paddedTopAndBottom:String = "."*lineWidth + "\n" + input + "."*lineWidth + "\n"
  val paddedAllSides = paddedTopAndBottom.linesIterator.map("." + _ + ".\n")
  paddedAllSides.mkString
...
def isSymbol(input: String) : Boolean =
  val isConnectedToSymbol = input match {
    case "*" => true
    case _ => false
  }

  isConnectedToSymbol.booleanValue()

def isDigit(input: String) : Boolean =
  val isConnectedToDigit = input match {
    case numericPattern(input) => true
    case _ => false
  }

  isConnectedToDigit.booleanValue()

def parseConnections(padding: Int, x: Int, y: Int, input: String) :  (String, Boolean, Boolean, Int) =
  val currentIndex = x + (y*padding)
//  println("central index " + currentIndex + " is char " + input(currentIndex))

  val symbolScanResult = directions.map( direction =>
    val indexToScan = currentIndex + direction(0) + (direction(1) * padding)
    val neighbourChar = input(indexToScan)
    val isStar = isSymbol(neighbourChar.toString)
//    println(s"surrounding index $indexToScan returns $neighbourChar and is a star $isStar")
    (indexToScan, isStar)
  )

  val isFinalDigit = !isDigit(input(currentIndex + 1).toString)

  // if the scanned char has a connection star then get the index of that star


//  println("symbolScanResult.count(_._2) = " + symbolScanResult.count(_._2))
//  println("symbolScanResult " + symbolScanResult.exists(_._2))
  if (symbolScanResult.count(_._2) > 0) {
    (input(currentIndex).toString, symbolScanResult.exists(_._2), isFinalDigit, symbolScanResult.filter(_._2).head._1)
  }
  else {
    (input(currentIndex).toString, symbolScanResult.exists(_._2), isFinalDigit, 0)
  }

def parse(line: String): List[String] = numPattern.findAllIn(line).toList
//
//def solution(input: String, summarise: Summary): Int =
//  input.linesIterator.map(parse andThen summarise).sum

def getAllPartsFromSchematic(input: String) : List[List[String]] =
  input.linesIterator.map(parse).toList


def removeUnconnectedParts(input: String): Unit =
  val allPartNumbers = getAllPartsFromSchematic(input).flatten



// use the regex on `address`
//val schematicLine = "467.*114..334.467+"
//val match1 = numPattern.findAllIn(schematicLine).toList
//println(match1)
//
//// remove the last instance of 467
//println(match1.zipWithIndex.filter(_._2 != match1.lastIndexOf("467")).map(_._1))
//
//// remove the last instance of 114
//println(match1.zipWithIndex.filter(_._2 != match1.lastIndexOf("114")).map(_._1))

val exampleSchematic = "467..114..\n...*......\n..35..633.\n......#...\n617*......\n.....+.58.\n..592.....\n......755.\n...$.*....\n.664.598..\n"

def solution(padding: Int, input: String, lineWidth: Int) : scala.collection.mutable.Map[Int, List[Int]] =
  var listOfPartNumber = scala.collection.mutable.ListBuffer.empty[Int]
  var partsMap = scala.collection.mutable.Map.empty[Int, List[Int]]
  var tempPartNumber = scala.collection.mutable.ListBuffer.empty[String]
  var tempPartConnection = scala.collection.mutable.ListBuffer.empty[Boolean]
  var tempPartConnectionStarIndex = scala.collection.mutable.ListBuffer.empty[Int]

  for(row <- 1 until input.linesIterator.length-1) {
    for(col <- 1 until lineWidth-1) {
      val result = parseConnections(padding, col, row, input)

      // if current char is a digit then store the digit and if connected
      if (result._1.charAt(0).isDigit) {
//        println("result " + result)
        tempPartNumber += result._1 // the digit
        tempPartConnection += result._2 // is the overall number connected to a star
        tempPartConnectionStarIndex += result._4

        // if its final digit then check if any connection and create the part number
        // then if its connected , store in listOfPartNumber
        if (result._3) { // is it the final digit, if so lets collect all the data for the part number
//          println("tempPartNumber " + tempPartNumber)
//          println("tempPartConnection " + tempPartConnection)
//          println("tempPartConnectionStarIndex " + tempPartConnectionStarIndex.max)

          // tempPartNumber is now the complete number as its the final digit
          // if its a connected to a star then store the part number in a list
          if(tempPartConnection.contains(true)) {
            listOfPartNumber += tempPartNumber.mkString.toInt

            if(partsMap.contains(tempPartConnectionStarIndex.max)) {
              val foundGears = partsMap(tempPartConnectionStarIndex.max):::List(tempPartNumber.mkString.toInt)
              partsMap += (tempPartConnectionStarIndex.max, foundGears)
            } else {
              partsMap += (tempPartConnectionStarIndex.max, List(tempPartNumber.mkString.toInt))
            }
          }

          // reset for the next numbers
          tempPartNumber = scala.collection.mutable.ListBuffer.empty[String]
          tempPartConnection = scala.collection.mutable.ListBuffer.empty[Boolean]
          tempPartConnectionStarIndex = scala.collection.mutable.ListBuffer.empty[Int]
        }
      }
    }
  }
  partsMap.filter(_._2.length>1)

println(solution(13, extendSchematic(exampleSchematic, 10), 12).values.map(_.product).sum)

//HashMap(123 -> List(755, 598), 30 -> List(467, 35))

val puzzleInput = "...........................775.651...............887....79...946...921...493.....942..942.....151....155....................................\n" +
  "......240...................*.....-......................$..*...................*.......%.....+....................956.549.*290.......834...\n" +
...
println(extendSchematic(puzzleInput,140))
println(solution(143, extendSchematic(puzzleInput, 140), 142).values.map(_.product).sum)
