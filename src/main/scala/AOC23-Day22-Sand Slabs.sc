...
case class Brick(coord1: (Int,Int,Int), coord2: (Int,Int,Int), supporting: Set[Brick],supportedBy: Set[Brick]) {
  def z: Int =
    Math.min(coord1._3, coord2._3)

  def height: Int =
    Math.abs(coord1._3 - coord2._3)
}
...
def parse(input: String): List[Brick] =
  input.linesIterator.map(line=>
    ...
    Brick((coord1(0), coord1(1), coord1(2)),(coord2(0), coord2(1), coord2(2)), Set(), Set())
  ).toList
...
def drop(domain: HashMap[Int, Set[Brick]], bricks:List[Brick], brickIndexToProcess: Int): HashMap[Int, Set[Brick]] =

  val brick = bricks(brickIndexToProcess)

  // handle the first ever brick
  if domain.isEmpty then
    // when adding to the domain we need to add for both min and max Z values
    val groundLevel = 1
    val levelOfTopOfBrick = 1 + brick.height
    val updatedDomain = domain + (groundLevel -> Set(brick)) + (levelOfTopOfBrick -> Set(brick))

    if brickIndexToProcess < bricks.size -1 then
      drop(updatedDomain, bricks, brickIndexToProcess+1)
    ...
    domain.keys.toList.sorted.reverse.foreach( level =>
      ...
      bricksAtCurrentLevel.foreach( otherBrick =>
        val levelOfTopOfBrick = level + brick.height
        val isOverlapping = overlapping(brick, otherBrick)
        if isOverlapping then
          // store the brick on level above current level
          val updatedDomain = if !domain.contains(level+1) then domain + (level+1 -> Set[Brick]()) else domain
          val updatedDomain2 = if !updatedDomain.contains(levelOfTopOfBrick+1) then updatedDomain + (levelOfTopOfBrick+1 -> Set[Brick]()) else updatedDomain


          val updatedSupportedBy = brick.supportedBy + otherBrick
          val updatedBrick = brick.copy(supportedBy = updatedSupportedBy)

          val updatedSupporting = otherBrick.supporting + brick
          val updatedSupportingBrick = otherBrick.copy(supporting = updatedSupporting)

          val updatedDomainLevel = domain(level+1) + updatedBrick
          val updatedDomain3 = domain + (level + 1 -> updatedDomainLevel)
  ...
  domain.keys.toList.sorted.reverse.foreach( level =>
    ...
    bricksAtCurrentLevel.foreach( otherBrick =>
      ...
      if isOverlapping then
        // store the brick on level above current level
        if !domain.contains(level+1) then domain(level+1) = Set[Brick]()
...
//        brick.updateFallenToZ(level+1)
        ...
        if !domain.contains(level + 1 + brick.height) then domain(level + 1 + brick.height) = Set[Brick]()
...
def countDisintegrated(domain: HashMap[Int, Set[Brick]]): Unit =
  val cannotBeDisintegrated = Set[Brick]()
...
def printSummary(domain: HashMap[Int, Set[Brick]]): Unit = {
...
def findLongestDisintegrationSequence(domain: HashMap[Int, Set[Brick]]): Unit =
  val queue = mutable.Queue[Brick]()

  var currentDisintegratedChain = 0
  val allBricks = domain.values.flatten.toSet

  queue.addOne(domain(2).head)
...
//  allBricks.foreach(brick =>
//    currentDisintegratedChain = 0
//    scanSupportedBrick(brick)
//
//    maxDisintegrated += currentDisintegratedChain
//  )
//  println(f"Max disintegrated: $maxDisintegrated")
...
//  val domain = HashMap[Int, Set[Brick]]()
  ...
  val orderedBricks = bricks.sortBy(brick => brick.z)
  val domain = drop(HashMap[Int,Set[Brick]](), orderedBricks, 0)
    //  val newScores = currentScores + (score -> (currentScores.getOrElse(score, 0) + 1))

  //  printSummary()

  countDisintegrated(domain)


//def partTwo(input: String): Unit =
//  // K = level, V = list of bricks sitting on that level
//  var domain = HashMap[Int, Set[Brick]]()
//  val bricks = parse(input)
//
//  // order by the lowest z coord
//  // start with the lowest brick and drop until it hits the ground/another brick
...
//    }.foreach(brick =>
//      domain = drop(domain, brick._2)
//    )
//
////  printSummary()
//
//  findLongestDisintegrationSequence(domain)

//  println(domain(1).head.supporting)
//  val domain2 = domain.clone()
//  domain2(1).head.addSupporting(new Brick((1,1,1),(1,1,1)))
//  println(domain(1).head.supporting)
//  println(domain2(1).head.supporting)
