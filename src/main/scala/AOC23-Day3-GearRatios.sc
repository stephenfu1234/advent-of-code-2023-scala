...
def isSymbol(input: String) : Boolean =
  val isConnectedToSymbol = input match {
    case numericPattern(input) => false
    case "." => false
    case _ => true
  }
...
def parseConnections(padding: Int, x: Int, y: Int, input: String) :  (String, Boolean, Boolean) =
...
//    println(s"surrounding index $indexToScan returns $neighbourChar")
    isSymbol(neighbourChar.toString)
  )

  val isFinalDigit = !isDigit(input(currentIndex + 1).toString)

  (input(currentIndex).toString, symbolScanResult.contains(true), isFinalDigit)
...
def solution(padding: Int, input: String, lineWidth: Int) : List[Int] =
  ...
  for(row <- 1 until input.linesIterator.length-1) {
    for(col <- 1 until lineWidth-1) {
      ...
      if (result._1.charAt(0).isDigit) {
        tempPartNumber += result._1
        tempPartConnection += result._2
  ...
  listOfPartNumber.toList

println(solution(13, extendSchematic(exampleSchematic, 10), 12).sum)
...
println(solution(143, extendSchematic(puzzleInput, 140), 142).sum)
